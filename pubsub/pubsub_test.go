...
func TestCbPubSub_Publish_GroupNotFound(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	err := pubsub.Publish(context.Background(), "test-message")
	if err == nil {
		t.Error("Publish should return error when group not found")
	}
	if err.Error() != "publish error, group not found" {
		t.Errorf("Publish error = %v, want 'publish error, group not found'", err)
	}
}
	...
	mockRepo.EXPECT().
		...
		DoAndReturn(func(ctx context.Context, key string, paths []string) error {
			...
			expectedPaths := map[string]bool{
				"test-group.inactive-instance": true,
				"other-group.another-inactive": true,
			}
...
func TestCbPubSub_Assign_Success(t *testing.T) {
	...
	expectedTTL := time.Duration(pubsub.cfg.SelfDocTTLSeconds) * time.Second
	mockRepo.EXPECT().
		Upsert(gomock.Any(), pubsub.selfDocId, gomock.Any(), expectedTTL).
		Return(nil)

	expectedPath := util.GetAssignmentPath(pubsub.groupName, pubsub.instanceId)
	mockRepo.EXPECT().
...
func TestNewCbPubSub_ConfigDefaults(t *testing.T) {
	...
	_, err := NewCbPubSub[string]("test-group", cfg)
	if err == nil {
