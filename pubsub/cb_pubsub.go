...
func (c *cbPubSub[T]) Publish(ctx context.Context, msg T) error {
	...
	group, found := allDoc[c.groupName]
	if !found {
		return fmt.Errorf("publish error, group not found")
	}

	for member, _ := range group {
...
func (c *cbPubSub[T]) doSubscribe(ctx context.Context, handler PubSubHandler[T]) error {
	...
	for {
		...
		case <-ticker.C:
			...
			selfDocTTL := time.Duration(c.cfg.SelfDocTTLSeconds) * time.Second
			...
			err := util.WithRetry(ctx, c.subscribeRetryConfig, func() error {
				...
				if errors.Is(err, gocb.ErrDocumentNotFound) {
					c.logger.Info("self document not found, recreating...", "instance_id", c.instanceId, "group_name", c.groupName)
					return c.assign(ctx)
				}
				return err
			})
			...
			if err != nil {
				c.logger.Error("failed to get self document after retries, shutting down", "error", err, "instance_id", c.instanceId, "group_name", c.groupName)
...
func (c *cbPubSub[T]) Close() error {
	...
	err := c.shutdownMgr.Shutdown(func(shutdownCtx context.Context) {
		if c.repository != nil {
			...
			pathToRemove := util.GetAssignmentPath(c.groupName, c.instanceId)
...
func (c *cbPubSub[T]) performCleanup(ctx context.Context) error {
	...
	for groupName, memberMap := range allDoc {
		for memberId, _ := range memberMap {
			var res interface{}
			_, err = c.repository.Get(ctx, fmt.Sprintf("%s%s", constant.SelfDocPrefix, memberId), &res)
			if errors.Is(err, gocb.ErrDocumentNotFound) {
				c.logger.Debug("inactive member detected", "member_id", memberId, "group_name", groupName)
				inactiveMembers = append(inactiveMembers, fmt.Sprintf("%s.%s", groupName, memberId))
			}
		}
	}
...
func (c *cbPubSub[T]) assign(ctx context.Context) error {
	...
	err = c.repository.UpsertPath(ctx, constant.AssignmentDocName, util.GetAssignmentPath(c.groupName, c.instanceId), currentTimestamp)
	if err != nil {
		return err
	}
...
func NewCbPubSub[T any](groupName string, cfg config.PubSubConfig) (PubSub[T], error) {
	cfg.ApplyDefaults()

	id := uuid.NewString()
	logger := util.NewLogger("cb-pubsub").With("instance_id", id, "group_name", groupName)
