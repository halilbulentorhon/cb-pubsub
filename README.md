...
- ⏱️ **TTL Management**: Built-in document TTL handling
...
```bash
go get github.com/halilbulentorhon/cb-pubsub
```
...
func main() {
    cfg := config.PubSubConfig{
        ...
        SelfDocTTLSeconds:      30, // Optional, defaults to 30 seconds  
    ...
    ps, err := pubsub.NewCbPubSub[string]("my-group", cfg)
    if err != nil {
        panic(err)
    }
    defer ps.Close()
    ...
    // Publish a message (note: group is set during NewCbPubSub)
...
ps, err := pubsub.NewCbPubSub[MyMessage]("custom-group", cfg)
if err != nil {
    panic(err)
}
...
type PubSubConfig struct {
    CouchbaseConfig        CouchbaseConfig `json:"couchbase"`
    PollIntervalSeconds    int             `json:"pollIntervalSeconds"`    // Defaults to 1
    SelfDocTTLSeconds      int             `json:"selfDocTTLSeconds"`      // Defaults to 30
...
2. **Assignment Document**: A global assignment document (`_pubsub_all`) tracks all active instances per group
3. **Instance Documents**: Each instance has its own document (`_pubsub_instance_{uuid}`) containing messages
4. **Message Delivery**: Publishers append messages to all active instances in the target group
...
7. **TTL Management**: Instance documents are automatically expired if not refreshed
